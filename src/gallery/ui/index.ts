...
export default class UI {
  ...
  constructor() {
    ...
    this.doms = {
      ...
      boards_content: document.querySelector(
  ...
  handleClick(e: MouseEvent) {
    if (e.target instanceof HTMLElement) {
      // Define event handler strategy for element clicks
      const MAP_EVENT = [
        {
          verify: () => {
        ...
        },
        {
          verify: () => {
            return this._isBInA(
              ['boards-info-close', 'boards-info'],
              (e.target as HTMLElement).classList.value.split(' ')
            );
          },
